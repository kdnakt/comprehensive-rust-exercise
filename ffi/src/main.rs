...
impl DirectoryIterator {
    fn new(path: &str) -> Result<DirectoryIterator, String> {
        unsafe {
            ffi::opendir(CString::new(path).unwrap().as_ptr())
                .as_mut()
                .map(|dir| DirectoryIterator {
                    path: CString::new(path).unwrap(),
                    dir,
                })
                .ok_or_else(|| format!("Failed to open directory: {path}"))
        }
    }
}
...
impl Iterator for DirectoryIterator {
    ...
    fn next(&mut self) -> Option<OsString> {
        unsafe {
            ffi::readdir(self.dir).as_ref().map(|entry| {
                // CStr を OsStr に変換します。
                let c_str = unsafe { CStr::from_ptr(entry.d_name.as_ptr()) };
